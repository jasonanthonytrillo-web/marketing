...
export default function SuperAdminDashboard() {
  ...
  const handleUpdateSettings = async (e) => {
    e.preventDefault();
    if (!tenantToEdit || !editName.trim()) return;
  ...
  return (
    <div className="h-screen bg-slate-950 text-white flex flex-col md:flex-row overflow-hidden">
                ...
                {tenants.map(tenant => (
                  <tr key={tenant.id} className="hover:bg-white/5 transition-colors group">
                    ...
                    <td className="p-6 text-right whitespace-nowrap">
                      ...
                      <span className="text-white/10 mr-3">|</span>
                      <button 
      ...
      {tenantToEdit && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/80 backdrop-blur-md">
          <div className="bg-slate-900 w-full max-w-md rounded-[40px] overflow-hidden shadow-2xl border border-white/10 animate-scale-in" onClick={e => e.stopPropagation()}>
            <div className="p-10">
              ...
              <form onSubmit={handleUpdateSettings} className="space-y-6">
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 px-1 font-bold">Store Name</label>
                  <input 
                    type="text" required
                    className="w-full bg-slate-800 border border-white/5 rounded-2xl px-6 py-4 text-sm text-white focus:border-indigo-500 outline-none transition-all placeholder-slate-600 font-bold"
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 px-1 font-bold">Brand Primary Color Theme</label>
                  <div className="flex gap-4 items-center">
                    <input 
                      type="color" 
                      className="w-14 h-14 bg-slate-800 border border-white/10 rounded-xl cursor-pointer p-1"
                      value={editColor}
                    ...
                    <div className="flex-grow">
                      <input 
                        type="text" required
                        className="w-full bg-slate-800 border border-white/5 rounded-2xl px-6 py-4 text-sm text-white focus:border-indigo-500 outline-none transition-all font-mono font-bold"
                        value={editColor}
                        onChange={e => setEditColor(e.target.value)}
                        placeholder="#f97316"
                      />
                ...
                <div className="flex gap-4 pt-4">
                  <button 
                    ...
                    className="flex-1 py-4 bg-slate-800 hover:bg-slate-700 text-white rounded-2xl font-bold transition-all text-xs uppercase tracking-widest"
                  >
                    Cancel
                  </button>
                  <button 
                    ...
                    className="flex-1 bg-indigo-600 text-white font-black py-4 rounded-2xl shadow-xl hover:bg-indigo-500 transition-all uppercase tracking-widest text-xs disabled:opacity-50"
                  >
                    {updatingName ? 'Updating...' : 'Save Settings'}
                  </button>
                </div>
              </form>
            </div>
