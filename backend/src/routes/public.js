...
router.get('/tenant/:slug/og-image', async (req, res) => {
  try {
    ...
    if (ogImage.startsWith('/')) {
      const host = req.headers.host || 'hometownbrew.jasonanthonytrillo.workers.dev';
...
router.get('/:slug', async (req, res) => {
  try {
    ...
    const redirectUrl = `https://hometownbrew.jasonanthonytrillo.workers.dev/menu`;
    ...
    if (ogImage && ogImage.startsWith('/')) {
      ogImage = `https://hometownbrew.jasonanthonytrillo.workers.dev${ogImage}`;
    }
